...
pub fn main(init: std.process.Init.Minimal) noreturn {
    ...
    defineMemoryLoadOpcodeHandlers(&asm_writer);
    defineMemoryStoreOpcodeHandlers(&asm_writer);
    defineMemoryManagementOpcodeHandlers(&asm_writer);
...
fn defineSupportRoutines(as: *AsmWriter) void {
    {
        // TODO: Could detect in build script that LLVM backend is used, meaning RegCall calling
        // convention could be used instead of System V
        var trampoline = as.startFunction("opcodeHandlerTrampoline", .{ .alignment = .@"64" });
        as.writeInstrs(&.{
    ...
    {
        var oof = as.startFunction("outOfFuelHandler", .{
            .binding = .global,
            .alignment = .@"16",
        });
        AsmWriter.OpcodeHandler.writeStartingCfiDirectives(as);
...
fn defineControlOpcodeHandlers(as: *AsmWriter) void {
    {
        var handler = as.defineOpcodeHandler(.{ .byte = .@"unreachable" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var nop = as.defineOpcodeHandler(.{ .byte = .nop }, .@"16");
        nop.jmpToNextHandler(as);
        nop.end(as);
    }
    for (&[2]opcodes.ByteOpcode{ .block, .loop }) |opcode| {
        var op = as.defineOpcodeHandler(.{ .byte = opcode }, .@"32");
        var block_type = SkipUlebIdx.fastPath(as, .r11, "type");
        as.writeInstrs(&.{"# no-op"});
        op.jmpToNextHandler(as);
        block_type.writeSlowPath(as);
        op.end(as);
    }
    {
        var @"if" = as.defineOpcodeHandler(.{ .byte = .@"if" }, .@"32");
    ...
    {
        var @"else" = as.defineOpcodeHandler(.{ .byte = .@"else" }, .@"32");
        as.printInstrs(&.{
    ...
    {
        var end = as.defineOpcodeHandler(.{ .byte = .end }, .@"32");
    ...
    {
        var br = as.defineOpcodeHandler(.{ .byte = .br }, .@"32");
        as.printInstrs(&.{
    ...
    {
        var br_if = as.defineOpcodeHandler(.{ .byte = .br_if }, .@"32");
        var false_branch = as.label(&.{"false"});
        as.printInstrs(&.{
    ...
    {
        var br_table = as.defineOpcodeHandler(.{ .byte = .br_table }, .@"32");
        as.printInstrs(&.{
    ...
    {
        var @"return" = as.defineOpcodeHandler(.{ .byte = .@"return" }, .@"32");
        as.printInstrs(&.{
...
fn defineCallOpcodeHandlers(as: *AsmWriter) void {
    ...
    for (&[2]struct { opcodes.ByteOpcode, []const u8, bool }{
        ...
        as.printInstrs(&.{
            ...
            .vsp = Gpr.vsp,
            .module = Gpr.module,
            .module_info_off = offsets.module_inst.module,
            .info_types_off = offsets.module.types,
            .module_tables_off = offsets.module_inst.tables,
            .table_len_off = offsets.table_inst.len,
            .oob = oob,
            .null_elem = null_elem,
        });
        for (
...
fn defineParametericOpcodeHandlers(as: *AsmWriter) void {
    ...
    {
        var select_t = as.defineOpcodeHandler(.{ .byte = .@"select t" }, .@"16");
        AsmWriter.OpcodeHandler.writeStartingCfiDirectives(as);
    ...
    {
        var select = as.defineOpcodeHandler(.{ .byte = .select }, .@"16");
...
fn defineLocalOpcodeHandlers(as: *AsmWriter) void {
    for (&[3]opcodes.ByteOpcode{ .@"local.get", .@"local.set", .@"local.tee" }) |opcode| {
        var op = as.defineOpcodeHandler(.{ .byte = opcode }, .@"64");
...
fn defineGlobalOpcodeHandlers(as: *AsmWriter) void {
    {
        var get = as.defineOpcodeHandler(.{ .byte = .@"global.get" }, .@"64");
        ...
        as.printInstrs(&.{
            ...
            .module = Gpr.module,
            .module_info_off = offsets.module_inst.module,
            .module_globals_off = offsets.module_inst.globals,
            .info_types_off = offsets.module.global_types,
            .global_type_size = 2,
    ...
    {
        var set = as.defineOpcodeHandler(.{ .byte = .@"global.set" }, .@"64");
        var jump_table = as.label(&.{"jump_table"});
...
fn defineTableAccessOpcodeHandlers(as: *AsmWriter) void {
    {
        var get = as.defineOpcodeHandler(.{ .byte = .@"table.get" }, .@"32");
    ...
    {
        var set = as.defineOpcodeHandler(.{ .byte = .@"table.set" }, .@"32");
        var table_idx = DecodeUlebIdx.fastPath(as, .r13, .{ .r14, .r15 }, "table");
        var oob = as.label(&.{"oob"});
        as.printInstrs(&.{
...
fn defineMemoryLoadOpcodeHandlers(as: *AsmWriter) void {
    for (&[_]struct { opcodes.ByteOpcode, std.mem.Alignment, []const u8, Gpr.Size }{
...
fn defineMemoryStoreOpcodeHandlers(as: *AsmWriter) void {
    for (&[_]struct { opcodes.ByteOpcode, std.mem.Alignment, Gpr.Size, []const u8, Gpr.Size }{
...
fn defineMemoryManagementOpcodeHandlers(as: *AsmWriter) void {
    {
        var memory_size = as.defineOpcodeHandler(.{ .byte = .@"memory.size" }, .@"32");
        ...
        }, .{ .mems = Gpr.mems, .mem_size_off = offsets.mem_inst.size, .vsp = Gpr.vsp });
        memory_size.jmpToNextHandler(as);
        mem_idx_decode.writeSlowPath(as);
        memory_size.end(as);
    }
    {
        var memory_grow = as.defineOpcodeHandler(.{ .byte = .@"memory.grow" }, .@"64");
        var mem_idx_decode = DecodeUlebIdx.fastPath(as, .r11, .{ .r13, .r14 }, "memory");
        ...
        as.printInstrs(&.{
            ...
            // .mem_base_off = offsets.mem_inst.base,
            .mem_size_off = offsets.mem_inst.size,
            .mem_cap_off = offsets.mem_inst.capacity,
            .mem_limit_off = offsets.mem_inst.limit,
...
fn defineConstOpcodeHandlers(as: *AsmWriter) void {
    for (&[_]struct { opcodes.ByteOpcode, u5, IntType, u7 }{
        .{ .@"i32.const", 5, .i32, 32 },
        .{ .@"i64.const", 10, .i64, 64 },
    }) |info| {
        const opcode, const max_byte_len, const int_type, const bit_size = info;
        var op = as.defineOpcodeHandler(.{ .byte = opcode }, .@"64");
    ...
    {
        var op = as.defineOpcodeHandler(.{ .byte = .@"f32.const" }, .@"64");
        as.printInstrs(&.{
    ...
    {
        var op = as.defineOpcodeHandler(.{ .byte = .@"f64.const" }, .@"64");
        as.printInstrs(&.{
...
const OpcodeNamePrefix = struct {
    ...
    fn name(prefix: *const OpcodeNamePrefix, s: []const u8) AsmWriter.Opcode {
        const final_len = prefix.prefix_len + s.len;
        @memcpy(prefix.buf[prefix.prefix_len..final_len], s);
        const final_name = prefix.buf[0..final_len];
        return .{
            .byte = std.meta.stringToEnum(opcodes.ByteOpcode, final_name) orelse
                std.debug.panic("no byte opcode named {s}", .{final_name}),
        };
    }
};
...
fn defineIntegerOpcodeHandlers(as: *AsmWriter, int_type: IntType) void {
    ...
    {
        var eqz = as.defineOpcodeHandler(opcode_name.name(".eqz"), .@"64");
        as.printInstrs(&.{
    ...
    for (&[_][2][]const u8{
        ...
        var cmp = as.defineOpcodeHandler(opcode_name.name(info[0]), .@"64");
        as.printInstrs(&.{
    ...
    {
        var add = as.defineOpcodeHandler(opcode_name.name(".add"), .@"64");
        as.printInstrs(&.{
    ...
    {
        var sub = as.defineOpcodeHandler(opcode_name.name(".sub"), .@"64");
        as.printInstrs(&.{
            "mov {[r13]f}, {[size]t} ptr [{[vsp]f} - 0x10]",
    ...
    {
        var mul = as.defineOpcodeHandler(opcode_name.name(".mul"), .@"64");
        as.printInstrs(&.{
            "mov {[r13]f}, {[size]t} ptr [{[vsp]f} - 0x10]",
    ...
    for (&[_]struct { []const u8, Gpr.Size }{
        ...
        var extend = as.defineOpcodeHandler(opcode_name.name(info[0]), .@"32");
        as.printInstrs(&.{
    ...
    if (int_type == .i64) {
        var extend = as.defineOpcodeHandler(.{ .byte = .@"i64.extend32_s" }, .@"32");
        as.printInstrs(&.{
...
fn defineFloatOpcodeHandlers(as: *AsmWriter, float_type: AsmWriter.FloatType) void {
    ...
    for (&[_]struct { []const u8, []const u8, u1 }{
        ...
        var cmp = as.defineOpcodeHandler(opcode_name.name(name), .@"64");
        as.printInstrs(&.{
    ...
    {
        var abs = as.defineOpcodeHandler(opcode_name.name(".abs"), .@"32");
        as.printInstrs(&.{
            "mov {[r13]f}, {[mask]s}",
    ...
    {
        var neg = as.defineOpcodeHandler(opcode_name.name(".neg"), .@"32");
        as.printInstrs(&.{
            "mov {[r13]f}, {[mask]s}",
...

    {
        ...
        var min = as.defineOpcodeHandler(opcode_name.name(".min"), .@"64");
        as.printInstrs(&.{
    ...
    {
        var max = as.defineOpcodeHandler(opcode_name.name(".max"), .@"64");
        as.printInstrs(&.{
            "mov{[int_suffix]c} xmm0, {[size]t} ptr [{[vsp]f} - 0x10] # first",
            "mov{[int_suffix]c} xmm1, {[size]t} ptr [{[vsp]f} - 0x20] # second",
            "movap{[float_suffix]c} xmm2, xmm0",
    ...
    {
        var copysign = as.defineOpcodeHandler(opcode_name.name(".copysign"), .@"64");
...
fn defineNumericConversionOpcodeHandlers(as: *AsmWriter) void {
    {
        var wrap = as.defineOpcodeHandler(.{ .byte = .@"i32.wrap_i64" }, .@"16");
    ...
    for (&[_]IntToFloat{
        ...
        var trunc = as.defineOpcodeHandler(.{ .byte = info.opcode }, .@"16");
    ...
    {
        var extend = as.defineOpcodeHandler(.{ .byte = .@"i64.extend_i32_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var extend = as.defineOpcodeHandler(.{ .byte = .@"i64.extend_i32_u" }, .@"16");
        as.printInstrs(&.{
            "xor r13d, r13d",
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f32.convert_i32_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f32.convert_i32_u" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f32.convert_i64_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f32.convert_i64_u" }, .@"16");
    ...
    {
        var demote = as.defineOpcodeHandler(.{ .byte = .@"f32.demote_f64" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f64.convert_i32_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f64.convert_i32_u" }, .@"16");
        as.printInstrs(&.{
            "mov r11d, dword ptr [{[vsp]f} - 0x10]",
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f64.convert_i64_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var convert = as.defineOpcodeHandler(.{ .byte = .@"f64.convert_i64_u" }, .@"16");
        as.write(
    ...
    {
        var demote = as.defineOpcodeHandler(.{ .byte = .@"f64.promote_f32" }, .@"16");
        as.printInstrs(&.{
    ...
    for (&[_]opcodes.ByteOpcode{
        .@"i32.reinterpret_f32",
        .@"i64.reinterpret_f64",
        .@"f32.reinterpret_i32",
        .@"f64.reinterpret_i64",
    }) |name| {
        var reinterpret = as.defineOpcodeHandler(.{ .byte = name }, .@"16");
        as.writeInstrs(&.{"# no-op"});
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i32.trunc_sat_f32_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i32.trunc_sat_f32_u" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i32.trunc_sat_f64_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i32.trunc_sat_f64_u" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i64.trunc_sat_f32_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i64.trunc_sat_f32_u" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i64.trunc_sat_f64_s" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var trunc = as.defineOpcodeHandler(.{ .fc = .@"i64.trunc_sat_f64_u" }, .@"16");
        as.printInstrs(&.{
...
fn defineReferenceOpcodeHandlers(as: *AsmWriter) void {
    {
        var ref_null = as.defineOpcodeHandler(.{ .byte = .@"ref.null" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var is_null = as.defineOpcodeHandler(.{ .byte = .@"ref.is_null" }, .@"16");
        as.printInstrs(&.{
    ...
    {
        var ref_func = as.defineOpcodeHandler(.{ .byte = .@"ref.func" }, .@"16");
...
fn defineBulkMemoryOpcodeHandlers(as: *AsmWriter) void {
    {
        var memory_init = as.defineOpcodeHandler(.{ .fc = .@"memory.init" }, .fromByteUnits(128));
        ...
        as.printInstrs(&.{
            ...
            .module = Gpr.module,
            .module_info_off = offsets.module_inst.module,
            .module_datas_drop_mask_off = offsets.module_inst.datas_drop_mask,
            .info_datas_ptrs_off = offsets.module.datas_ptrs,
            .info_datas_lens_off = offsets.module.datas_lens,
            .size_field_off = offsets.mem_inst.size,
            .vsp = Gpr.vsp,
            .oob = oob,
    ...
    for (&[_]struct { opcodes.FCPrefixOpcode, u8 }{
        .{ .@"data.drop", 56 },
        .{ .@"elem.drop", 64 },
    }) |info| {
        const opcode, const drop_mask_offset = info;
        const kind = @tagName(opcode)[0..4];
        var drop = as.defineOpcodeHandler(.{ .fc = opcode }, .@"32");
    ...
    {
        var memmove = as.defineOpcodeHandler(.{ .fc = .@"memory.copy" }, .fromByteUnits(128));
        as.printInstrs(&.{
    ...
    }
    {
        ...
        // to the stack.
        var memset = as.defineOpcodeHandler(.{ .fc = .@"memory.fill" }, .@"64");
        as.printInstrs(&.{"mov r15, {f} # save IP to first byte after opcode"}, .{Gpr.vip});
        var oob = as.label(&.{"oob"});
    ...
    {
        var table_init = as.defineOpcodeHandler(.{ .fc = .@"table.init" }, .@"64");
        as.printInstrs(&.{
    ...
    {
        var table_copy = as.defineOpcodeHandler(.{ .fc = .@"table.copy" }, .@"64");
        as.printInstrs(&.{
    ...
    {
        var table_grow = as.defineOpcodeHandler(.{ .fc = .@"table.grow" }, .@"32");
        ...
        as.printInstrs(&.{
            ...
            .module_tables_off = offsets.module_inst.tables,
            .table_len_off = offsets.table_inst.len,
            .table_cap_off = offsets.table_inst.capacity,
            .table_limit_off = offsets.table_inst.limit,
            .vsp = Gpr.vsp,
            .fail = fail,
            .within_capacity = within_capacity,
            .vip = Gpr.vip,
    ...
    {
        var table_size = as.defineOpcodeHandler(.{ .fc = .@"table.size" }, .@"32");
        var table_idx_decode = DecodeUlebIdx.fastPath(as, .r11, .{ .r13, .r14 }, "table");
        as.printInstrs(&.{
    ...
    {
        var table_fill = as.defineOpcodeHandler(.{ .fc = .@"table.fill" }, .@"64");
        as.printInstrs(&.{"mov r15, {f} # save IP to first byte after opcode"}, .{Gpr.vip});
        var table_idx = DecodeUlebIdx.fastPath(as, .r11, .{ .r13, .r14 }, "table");
        ...
        as.printInstrs(&.{
            ...
            "sub r14, r13 # how many bytes were written to become aligned",
            "shr r14, 3 # how many elems were written to become aligned",
...
const TakeBranch = @import("TakeBranch.zig");
